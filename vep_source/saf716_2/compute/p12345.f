...
c      add by jiyang july19,2012
c      func:  output all the results in one file named vep.res
      ...
      open(10,file='../data4_check/data1',status='old')
      read(10,*) np2
      do i=1,np2
      read(10,*)
      enddo
      read(10,*) nqu2
      close(10)
      open(10,file='../data4_check/amend.flavia.msh',status='old')
      read(10,*)
      read(10,*)
      do i=1,np2
...
c       the sel_nodes in the 'saf1_sel.node0'
      open(10,file='selnode.sort', status='old')
      read(10,*) 
      read(10,*) nns 
      do i=1,nns
      read(10,*) (nscoor(j,i),j=1,4)
      write(*,*) (nscoor(j,i),j=1,4)
      enddo
      close(10)
      open(40,file='selnode.velo',status='unknown',form='formatted')
      do i=1,nns
      du=u0(1,nscoor(1,i)+np2*(nlayer-1))
      dv=u0(2,nscoor(1,i)+np2*(nlayer-1))
      duv=sqrt(du*du+dv*dv)
      write(40,1500) (nscoor(j,i),j=1,4),duv*1000.0/5.0      
      enddo
      close(40)
...
c
      open(42,file='vep.res',status='unknown',form='formatted')
     ...
     +' Hexahedra Nnode 8'
      ...
      write(42,1700) 'nodeID','x_coor','y_coor','z_coor','u','v','w',
     +'dxx','dyy','dzz','j2','p','e'

      do i=1,knode
      write(40,1000) i,(coor3(j,i),j=1,3)
      write(42,1600) i,(coor3(j,i),j=1,3),(u0(j,i),j=1,3),
     + (u1(j,i),j=1,6)
      enddo
...
1600  format(i9,20es15.5)      
1700  format(A9,12A15)      
      end
