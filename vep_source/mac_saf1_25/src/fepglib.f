...
cc1000continue
      ...
      real*8 function det(x1,x2,x3,y1,y2,y3)
      implicit real*8 (a-h,o-z)
...
c20    continue
c      if (j.gt.9) then
c      write(*,*) 'Error, plot filename too long .......',name
c      write(*,*) ' the length of name must be less or equal 8 character'
c      stop 111
c      endif
c     read(*,'(a3)') ch3
c      name(j:j)='.'
...
c      end
c 
c      subroutine getchar(n,k,ch3)
c      implicit real*8 (a-h,o-z)
c      character ch3*3
c      if (n.eq.0) ch3(k:k) = '0'
c      if (n.eq.1) ch3(k:k) = '1'
c      if (n.eq.2) ch3(k:k) = '2'
c      if (n.eq.3) ch3(k:k) = '3'
c      if (n.eq.4) ch3(k:k) = '4'
c      if (n.eq.5) ch3(k:k) = '5'
c      if (n.eq.6) ch3(k:k) = '6'
c      if (n.eq.7) ch3(k:k) = '7'
c      if (n.eq.8) ch3(k:k) = '8'
c      if (n.eq.9) ch3(k:k) = '9'
c      return
c      end
