...
c*********************************************************************c
       ...
       common /timer1/timio(3),timges(3),timar(3),timcmn(3),timitr(3)
       common /timer2/timusr(3),timsys(3),timall(3),timtot(3)
       dimension timer(9,numblk),timeriblk(9)
...
c        
     ...
     +             //'   sys      all     tot'
       do iblk = 1,numblk-1
       write(nn,9) iblk,(timer(i,iblk),i = 1,9)
       end do
       write(nn,*) '**************************************************'
c
...
c
       do i = 1,9
       balance(i) = 0.d0
       timermax(i) = timer(i,1)
       nodmax(i) = 1
       timermin(i) = timer(i,1)
       nodmin(i) = 1
       do iblk = 2,numblk-1
       if(timer(i,iblk).gt.timermax(i)) then
       timermax(i) = timer(i,iblk)
       nodmax(i) = iblk
       end if
       if(timer(i,iblk).lt.timermin(i)) then
       timermin(i) = timer(i,iblk)
       nodmin(i) = iblk
       end if
       end do
